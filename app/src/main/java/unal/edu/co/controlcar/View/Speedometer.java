...
public class Speedometer extends View {
	...
	protected void onSizeChanged(int width, int height, int oldw, int oldh) {
		...
		if (width > height){
			radius = (float)7*height/10;
		}else{
			radius = (float)7*width/10;
		}
...
//		Log.d(TAG, "Height spec: " + MeasureSpec.toString(heightMeasureSpec));
		...
		setMeasuredDimension(chosenDimension, 55*chosenDimension/100);
	}
	...
	private void drawReading(Canvas canvas){
		Path path = new Path();
		String message = String.format("%s",String.valueOf(this.mCurrentSpeed)+" Km/h");
